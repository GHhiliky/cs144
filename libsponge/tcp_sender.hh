...
class RetransmissionTimer{
  ...
  public:
    ...
    void init_RTO(){_RTO=_initial_RTO;}
    ...
    bool expire(size_t ms_since_last_tick){
      if(_status==false)return false;

      _time+=ms_since_last_tick;
      if(_time>=_RTO)return true;
...
class TCPSender {
  private:
    ...
    uint16_t _count_retransmission;
